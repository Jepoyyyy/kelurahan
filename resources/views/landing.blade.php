...
@section('title', 'Kelurahan Simpang III Sipin')
...
{{-- ── LAYANAN ── --}}
<section class="mt-10 scroll-mt-24 min-h-screen" id="layanan">
        ...
        <div class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
            @foreach ($layanan as $index => $item)
                <a href="{{ url($item['url']) }}"
                    ...
                    <p class="font-bold text-[#1d70b8] leading-relaxed
                               underline decoration-[#1d70b8] underline-offset-5
                               group-hover:decoration-blue-900 group-hover:text-blue-900
                               group-hover:decoration-3 transition-all duration-150 visited:text-[#170C79]">
            ...
            <div class="flex flex-col">
                ...
                <template x-for="(item, index) in items" :key="item.url + index">
                    <a :href="item.url"
                        ...
                        <p class="font-bold text-[#1d70b8] leading-relaxed text-lg
                                   underline decoration-[#1d70b8] underline-offset-5
                                   group-hover:decoration-blue-900 group-hover:text-blue-900
                                   group-hover:decoration-3 transition-all duration-150">
                            <span class="inline active:bg-teal-300" x-text="item.judul"></span>
                        </p>

                        <p class="text-lg text-gray-500 leading-relaxed line-clamp-2" x-text="item.excerpt"></p>
...
{{-- ── KEGIATAN ── --}}
<section class="mt-10 scroll-mt-24 min-h-screen" id="kegiatan">
    <div class="max-w-5xl mx-auto px-4 md:px-6">
        <div class="flex items-center justify-between gap-4 mb-6">
            <div class="flex items-center gap-3 flex-1">
                <h2 class="text-2xl md:text-3xl font-bold whitespace-nowrap">
        ...
        @forelse ($groupedEvents as $weekKey => $week)
            ...
            <div class="week-group">
                <div class="grid border-t-2 0 border-b bg-gray-50 py-3 mt-6 first:mt-0"
                    style="grid-template-columns: 130px 1fr">
                    <div class="font-mono text-base font-semibold text-[#1d70b8] tracking-tight
                                pr-4 border-r border-gray-300 flex items-center">
                        {{ $weekKey }}
                    </div>
                    <div class="font-sans text-base font-semibold text-gray-500 tracking-widest
                ...
                @else
                    @foreach ($week['events'] as $event)
                        <div class="grid py-3.5 border-b border-gray-200 last:border-b-0 relative transition-colors duration-100
                            ...
                            <div class="flex flex-col items-start pr-4
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    @if ($event->is_today)
                                        <span class="w-1.5 h-1.5 rounded-full text-xl bg-red-500 mb-0.5 shrink-0"
                                              title="Hari ini"></span>
                                    @endif
                                </div>
                                ...
                                @if ($event->is_today)
                                    <span class="font-mono text-sm font-semibold tracking-widest uppercase
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <span class="font-mono text-lg text-gray-400 flex items-center gap-1">
                                        <svg class="w-2.5 h-2.5 shrink-0" viewBox="0 0 16 16" fill="none"
                                            ...
                                            <path d="M8 2C5.24 2 3 4.24 3 7c0 4 5 5 9s5-5 5-9c0-2.76-2.24-5-5-5z" />
                                        </svg>
                                        {{ $event->lokasi }}
                                    </span>
                                </div>
            ...
            <div class="mt-8 pt-4 border-t border-gray-200 flex items-center justify-between gap-4 flex-wrap">
                <p class="text-lg text-black">
                    @if ($kalenderMeta['remaining'] > 0)
                        Menampilkan <span class="text-[#1d70b8] font-semibold">{{ $kalenderMeta['shown'] }} kegiatan</span>
                        terdekat &middot;
                    ...
                    @else
                        Menampilkan
                        <span class="text-[#1d70b8] font-semibold">{{ $kalenderMeta['shown'] }} kegiatan</span>
                        mendatang
                    @endif
                </p>
                <a href="#"
